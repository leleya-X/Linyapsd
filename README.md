...
| `LaunchApp` | `s -> s` | 在宿主上跑 `ll-cli run <应用 ID>` 把应用拉起来；恒返回空串 |
| `Quit` | `() -> s` | 让本实例退出，下次调用由 bus 重新拉起 |

调用方全部退出后它也会自己走，见下面「没人用了就自己退出」。

`LaunchApp` 跑的是写死的 `ll-cli run -- <应用 ID>`：子命令不接受参数，
调用方能决定的只有"跑哪个应用"。不经过 shell，参数原样交给 `execv`，
所以不存在命令注入；`--` 是给 ll-cli 的选项隔断，应用 ID 开头带个 `-`
也不会被当成它的选项。
...
`ll-cli run` 起完容器就把前台让给那个应用，一直到用户把它关掉才返回 ——
一次调用要挂几个小时。所以这里不可能"跑完再回复"，只能分出去就回。

恒返回空串是明说**"这个方法没有结果可报"，不等于"启动成功了"**。
应用 ID 写错、ll-cli 根本起不来，都不会从返回值上看出来。失败信息由 ll-cli
自己写在 linyapsd 的 stderr 上（`Error 1002: package not found:...` 那种就是），
由 bus 激活时它落进 bus 自己那套日志。Arch 上 dbus-broker 生成的是 systemd
瞬态单元，也就是 journal：

```bash
journalctl --user -u 'dbus-:1.2-io.github.leleya_x.Linyapsd@*.service'
```

交给 ll-cli 之后本进程就不再管它了：子进程另立会话（`setsid`），
linyapsd 因为"调用方都走了"收工时不会把已经交出去的应用一起带走。
...
**接口刻意做窄：没有任何能改动宿主数据的方法。** `ReadStates` / `ReadEntries`
只读文件；`LaunchApp` 只跑 `ll-cli run`，子命令写死，能变的只有"跑哪个应用" ——
`install`、`uninstall`、`upgrade` 这些会改系统状态的子命令在这里**没有入口**。
`Quit` 是唯一不碰外物的例外：它只关乎本进程自己的生命周期，
存在的理由是部署方换上新版本后，文件换了不等于新版本生效 ——
...
**出错不静默。** 除了 `LaunchApp`（它没有返回值可报，失败落在 stderr 上，见上），
其余方法读不到文件、D-Bus 回复发不出去，都会如实报错，
而不是回一个空结果、或让调用方一直干等到超时。

**安全性没有变化。** 这条链路本质上仍是「借宿主的某个进程在宿主命名空间里 fork」，
和直接写 systemd unit 一样是容器逃逸。D-Bus 只是让接口更规范、痕迹更少（不留 unit 文件、
不留中转文件、清理只需删两个文件），并不提升隔离性。

`LaunchApp` 也没有把口子开大，但它确实让这个逃逸**多了一种用法**：容器里的应用
现在可以拉起任意一个已安装的玲珑应用。子命令写死、只认应用 ID，改不了系统状态，
但"能启动别的程序"本身就是在别人的桌面会话里做事 —— 安装这个助手时要知道这一点。
